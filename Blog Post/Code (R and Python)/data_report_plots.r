...
setwd("/Users/mmoheebahmed/Desktop/UCD MA/Connected Politics Lab/Final Project/Code & Dataset")

#Loading base dataset


df <- read.csv("df_merged.csv")
...
subregions <- read.csv("/Users/mass/Documents/Masters/Courses/Connected Politics/Github repository/Content/Datasets and code/Datasets/Aux datasets/region_dataset.csv")
head(subregions)
...
# Creating Plot 1 for the blog: 


# Simplify category labels for blog readability
labels_clean <- c(
  "no_sdg_related" = "No SDG Mentions",
...
# Build blog-optimized plot
plot_blog <- ggplot(avg_by_year_long, aes(x = year, y = average, color = category, shape = category)) +
  geom_line(size = 1.5) +
  geom_point(size = 3.2) +
  geom_vline(xintercept = 2015, linetype = "dashed", color = "black") +
  annotate("text", x = 2015.1, y = max(avg_by_year_long$average) / 3,
           label = "SDGs Adopted (2015)", hjust = 0, size = 5, fontface = "bold") +
  scale_color_manual(values = colors, labels = labels_clean) +
  scale_shape_manual(values = shapes, labels = labels_clean) +
  labs(
    title = "Which SDG Topics Dominate UNGD Speeches?",
    subtitle = "SDG Topic Mentions in UNGD Speeches (2008 - 2023)",
    x = "Year",
    y = "Prevalence of SDG Topic (%)",
    color = "SDG Category",
    shape = "SDG Category"
...
ggsave("plot1_blog.pdf", plot = plot_blog, , dpi = 600, width = 19, height = 13, bg = "white")
...
gap_barplot <- ggplot(gap_summary, aes(x = mean_gap, y = topic_label, fill = mean_gap > 0)) +
  ...
  theme_minimal(base_size = 18) +
  theme(
    plot.title = element_text(size = 19, face = "bold", hjust = 0, margin = margin(b = 5)),
...
ggsave("plot5.pdf", plot = gap_barplot, width = 16, height = 10, bg = "white")
ggsave("plot5.png", plot = gap_barplot, width = 16, height = 10, bg = "white")
...
#____Creating Panel Regression Table
...
#______________Creating a Coefficient Plot___________
...
# Creat Coefficient Plot for Blog

coefficient_plot_blog <- ggplot(final_results_main, aes(x = estimate, y = reorder(topic_labels_short, estimate), color = significance)) +
  geom_point(size = 3) +
  geom_errorbarh(aes(xmin = estimate - 1.96 * std.error,
                     xmax = estimate + 1.96 * std.error), height = 0.2) +
  geom_vline(xintercept = 0, linetype = "dashed", color = "black") +
  ...
  labs(
    title = "Rhetoric vs. Results: Do UNGD Speeches Predict SDG Progress?",
    subtitle = "Tracking Promises against Actual Progress",
    x = "Impact on Implementation (Coefficient ± 95% CI)",
    y = " Grouped SDG Topics",
    color = "Statistical Significance"
  ) + 
...
theme(
    ...
    plot.subtitle = element_text(size = 18, hjust = 0.5, margin = margin(b = 15)),
    axis.title.x = element_text(color = "black", size = 16, face = "bold", margin = margin(t = 10)),
    axis.title.y = element_text (color = "black", size = 16, hjust = 0.5, face = "bold", margin = margin(r = 15)),
...
ggsave("plot6_blog.pdf", plot = coefficient_plot_blog, width = 13, height = 8, bg= "white")
